...
| **Exception: a display-only `sort`** | Sorting reorders, it does not change values or drop rows. "Top N" screens genuinely need it, and the alternative (an alphabetically charted ranking) is the more common failure |
| **Verification by restriction, not by policy** | `aggregate` / `filter` / `limit` / `derive` / `binTime` stay unreachable because no tool exposes them in this mode |
...
1. **Nothing in present mode may alter values or drop rows.** No aggregation, no
   filtering, no limiting, no derivation, no time binning.
2. **A sort is normalised into the spec** as a `transform_plan` step so the
   artifact stays replayable, but the model never writes a plan.
...
5. **Nothing here changes the natural-language mode** except that `submit_spec`
   gains an optional `sort` field *in present mode only*.
...
> The rows are final. Do not aggregate, filter, limit or derive anything — you
> have no tool that could. Look at the data, then choose the chart type, the two
> axes, the orientation, the title and any emphasis that best show what the user
> asked about.
...
## Task 4 — Display sort, and making an unchartable table fixable
...
- in present mode, `submit_spec({ chart, encodings, sort: { by, order } })` produces
  `spec.transform_plan.steps === [{ op: 'sort', by, order }]`;
- with no `sort`, the plan is `[]` and the incoming row order is preserved;
- `sort.by` naming a column that is not in the table is rejected with an actionable
  message (the model repairs);
- in `'ask'` mode the `sort` field is **not** in the tool schema, and passing it is
  ignored;
- **a follow-up keeps the sort** — see step 4.

**Step 2.** Add `sort` to the present-mode `submit_spec` schema only, and normalise
it in `validateSpec` into the plan.

**Step 3.** A present-mode call to `run_query` must fail *helpfully*: the tool
dispatcher should answer `` `run_query` is not available in present mode: the rows
you were given are final, and no tool can change them. Choose encodings that
present them instead.`` rather than the generic `unknown tool`.

**Step 4.** **Follow-ups stay stateless.** `recoverPlanFrom` currently reads only
`run_query` calls; in present mode it must also recover a `sort` submitted by an
earlier turn, otherwise a follow-up silently loses the ordering:

```ts
// recover, in order: a successful run_query's steps, else the last submit_spec sort
```

**Step 5 — the gap that needs re-encoding, not aggregation.** If the rows contain
two entries for the same category (e.g. two `GB` rows), the compiler refuses today
with "add an aggregate step" — which present mode cannot do. So present mode
validates the submitted **encodings against the actual rows** at submit time and,
when they would collide, rejects with guidance the model *can* act on:
...
**Step 6.** Verify: `test/loop.test.ts`, `test/present.test.ts`, the full suite.

**Commit:** `feat(agent): present-mode sort, and re-encoding instead of a dead end`
...
- reframe P0 items 1–3 in these terms (rank selection and named colours are still
  open; "a request we cannot honour must make a sound" now has its pipeline-level
  half done by the mode);
- downgrade structured semantic types to a *possible opt-in* for consumers who want
  the library to enforce aggregation rules, explicitly not required;
- add to **Refused by design**: "chartwright suggesting that a table would be
  better — presentation judgement belongs to the consumer".
...
1. In present mode the model can call **only** `describe_table`, `preview_rows` and
   ...
   unknown-tool error.
2. `aggregate`, `filter`, `limit`, `derive` and `binTime` are **unreachable** in
   present mode — by construction, not by instruction.
3. A display `sort` works, is recorded in the spec as a `transform_plan` step, and
...
- **The tricky part is called out:** the duplicate-category dead end (task 4 step 5)
  and the follow-up sort recovery (task 4 step 4) are the two things that would
  otherwise be found later, in use.
- **What this plan deliberately does not do:** structured semantic types, automatic
  colour ramps, consumer-supplied encodings overrides, and any suggestion that a
  table would be better than a chart.
