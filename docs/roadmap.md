...
| A measure channel must hold numbers, and a blank is a gap | A text column on `encodings.y` (or `y2`, `size`, `low`, `high`, or a point cloud's `x`) reached `Number()`, became `NaN`, serialised to `null`, and drew axes, a title and **no marks** with `warnings: []`. It is refused by name now, so the model repairs it in the same run. `Number(null)` was `0` besides, so every missing measure was drawn as a data point claiming zero — a null and a `0` were the same picture, which the `nulls-and-zeros` corpus case exists to forbid. Blanks leave gaps again; a measure that is neither blank nor a number is refused rather than drawn as an unasked-for hole. |
| `top_k` can mark its own complement | `rest: true` on a `top_k` rule styles the rows the ranked set excludes, so "highlight the top one and fade the rest" is two rules over **one** ranking rather than an approximation. There was no complement before, and the only construction that worked was an always-true threshold — a fact about a measure's sign, and a real threshold is a value the model is forbidden to fetch. So a model asked to fade the rest reached for a *larger* `top_k`, which counts from the top: on twelve rows `k: 11` faded ranks 1-11 including the winner an earlier rule had just highlighted, and left rank 12 as the only default-coloured bar, reading as the selected one. No warning, because neither rule was wrong alone. `rest` is taken over the same threshold and the same ties as the rule it mirrors, and an empty complement now warns instead of passing quietly. |
